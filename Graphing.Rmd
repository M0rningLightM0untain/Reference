---
title: "Graph Creation"
author: "Jack Sudds"
date: "2024-12-20"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

```{r Packages}
if (!require("pacman")) install.packages("pacman")
library(pacman)
p_load(
  #Learning & Base
  learnr, swirl, installr,
  #File & Package Management
  here, rio, openxlsx, renv, pacman, remotes, readxl,
  #Data Management
  tidyverse, linelist, naniar, parsedate, reshape2, dplyr,
  #Stats
  janitor, gtsummary, rstatix, broom, lmtest, easystats, skimr, spatstat,
  #Plots
  cowplot, RColorBrewer, ggnewscale, wesanderson, DiagrammeR, incidence2, gghighlight, ggtext, ggrepel, gganimate, ggsci, grates, gridExtra, ggforce, hexbin, plotly,
  #GIS
  sf, tmap, spdep, rnaturalearth, rnaturalearthdata, broom, ggmap, mapdata, maps, spData, BiocManager, geoR, giscoR,
  #Reports & Dashboards
  rmarkdown, reportfactory, officer, flexdashboard, shiny,
  #tables
  knitr, flextable, formattable, gt,
  #Other
  kableExtra, psych, BSDA, htmltools, rgl, lubridate, htmlwidgets,
  #Testing
  raster, dismo, terra, CCAMLRGIS, tigris, patchwork
)
```

```{r Test Data}
rm(list = ls(all.names = TRUE))
data1 <- read.xlsx(here("Data/data.xlsx"), sheet = "skeletal_part_abundances")
data2 <- read.xlsx(here("Data/data_2.xlsx"), sheet = "Sheep_Mandibles")
data3 <- iris
data4 <- mtcars

#wokring with data
#create right order for graphing in %>%
#arrange(match(period, c("Iron Age", "Roman", "Anglo-Saxon", "Medieval", "Post-Medieval", "Modern")))

```

```{r Basic Graphs and Aesthetic Options}
# Point plot
ggplot(data3, aes(x = Sepal.Length, y=Sepal.Width, colour = Species, shape = Species)) +
  geom_point() +
#Include line
  geom_vline(xintercept = 6, linewidth = 0.5, color = "orange") +
#Examples of the kinds of details which can be altered in a theme
  theme(
    plot.title = element_text(hjust = 0.5, face = 'bold'),
        plot.subtitle = element_text(hjust = 0.5),
        panel.background = element_rect(fill = 'white'),
        panel.grid = element_line(color = 'grey'),
        axis.line = element_line(color = 'black'),
        panel.grid.minor.x = element_line(color = 'white'),
        panel.grid.minor.y = element_line(color = 'grey'),
        panel.grid.major.y = element_line(color = 'darkgrey'),
    axis.text.x = element_text(angle = 90)
  ) +
#Specify details about legend
  guides(
#Specify text
    colour = guide_legend("guide title"),
#Remove guide
    #colour = "none"
#Alter order
    shape = guide_legend(order = 1),
  ) +
# Specify axis label and title text
  labs(
    title = "Title",
    subtitle = "Subtitle",
    x = "Complex Titles can contain other stuff, \u03B413C‰",
    y = expression("Log" [10]* " Ratio ")
  ) +
  scale_y_continuous(
#Breaks in axis
    breaks = seq(1.5,5,by = 0.2)
  ) +
  scale_x_continuous()
  #scale_color_brewer() +

# Bar chart using counts (..count.. is depreciated)

ggplot(data3, aes(x = Sepal.Length, fill = Species)) +
  geom_bar(stat = "count") +
#Second type of coord system
  coord_cartesian(
#Set axis limits
    xlim = c(4,8),
#Make bars sit on bottom row, also removes gaps
    expand = FALSE
  ) +
#Manually alter fill/colour related content
  scale_fill_manual(
#Change colours by including hex codes
    values = c('#1b9e77','#d95f02','#7570b3'),
#Change label contents
    labels = c("Setosa", "Versicolor", "Virginica")
    ) +
#Separate graph into collection of Smaller ones
  facet_wrap(
#Specify what to 
    ~Species,
#Scales can be individualised "free"/"free_x"
    scales = "free_y",
#direction of facet
      dir = "v",
#change location of label
      strip.position = "top") +
#Other Coords method
  coord_cartesian(xlim = c(3,9),
                  expand = FALSE) +
#Another Colour method
  #scale_fill_brewer(palette = "Dark2")


#Additional table in plot
#MWSTable <- data.frame(MWS = c("A", "B", "C", "D", "E", "F", "G", "H", "I"),
                       #Age_Min = c(0,5,7,12,24,30,61,86,154),
                       #Age_Max = c(5,7,12,24,30,61,86,154,"-"))
#annotation_custom(tableGrob(MWSTable, rows=NULL, theme = ttheme_default(base_size = 9)), xmin = "C", xmax = "E", ymin = 2, ymax = 7)

#Save previous plot
#ggsave("<filename.jpg/png>", dpi = <size>)
```

```{r Other Geoms}
#data
plotting <- data3
plotting_melt <- melt(data3, id.vars = "Species", variable.name = "Metric", value.name = "Value")
plotting_melt <- plotting_melt %>%
  arrange(desc(Metric))
plotting <- ggplot(plotting, aes(x=Sepal.Length, y = Sepal.Width, colour = Species))
plotting_melt_c <- ggplot(plotting_melt, aes(x=Metric, y = Value, colour = Species))
plotting_melt_f <- ggplot(plotting_melt, aes(x=Metric, y = Value, fill = Species))


#Jitter
plotting_melt_c +
  geom_jitter()+
  aes()+
  theme_linedraw()+
  labs(
    title = "Jitter plot"
  )

#fancy-er bars
plotting_melt_f +
  geom_bar(stat = "identity", position = position_dodge2(preserve = "single")) +
  theme(
    plot.title = element_text(hjust = 0.5, face = 'bold'),
        panel.background = element_rect(fill = 'white'),
        panel.grid.major.x = element_blank(),
        panel.grid.major.y = element_line(color = 'gray'),
        axis.line = element_line(color = "black"),
        panel.grid.minor.x = element_line(color = 'gray'),
        panel.grid.minor.y = element_line(color = 'gray'),
    plot.subtitle = element_text(hjust = 0.5)
    ) +
  labs(
    x = "Measurement",
    y = "Value",
    title = "Fancy-er Bars",
    fill = "Species"
  ) +
  scale_fill_manual(values = wes_palette(n = 3, name = "Darjeeling2"))
```

```{r Pie Chart - EXPAND}
pie_data <- data3
pie_data$SLfraction = pie_data$Sepal.Length / sum(pie_data$Sepal.Length)
pie_data$ymax = cumsum(pie_data$SLfraction)
pie_data$ymin = c(0, head(pie_data$ymax, n=-1))
pie_data$labelPosition = (pie_data$ymax + pie_data$ymin) / 2
pie_data$label = paste0(pie_data$Species, "\n Count: ", pie_data$Sepal.Length)

ggplot(pie_data, aes(ymax = ymax, ymin = ymin, xmax = 4, xmin = 3, fill = Species, stat = "identity")) +
  geom_rect() +
  theme_void() +
  coord_polar(theta = "y") +
  labs(
    title = "Sepal Length By Species"
  ) +
  xlim(c(2,4)) +
  scale_fill_brewer(palette = "Set3")
```

```{r Boxplot - EXPAND}
#data
box <- data3
box <- melt(box, id.vars = "Species", variable.name = "Metric", value.name = "Value")

#plotting
ggplot(box, aes(x = Metric, y = Value)) +
   geom_boxplot(outlier.alpha = 0) +
   stat_boxplot(geom = "errorbar", linetype = 1, width = 0.5) +
  aes() +
  theme_linedraw() +
  labs(
    x = "Metric",
    y = "Value",
    title = "Flower Box")

ggplot(box, aes(x = Metric, y = Value, fill = Species)) +
   geom_boxplot(outlier.alpha = 0) +
   stat_boxplot(geom = "errorbar", linetype = 1, width = 0.5) +
  aes() +
  theme_linedraw() +
  labs(
    x = "Metric",
    y = "Value",
    title = "Flower Box")
```
